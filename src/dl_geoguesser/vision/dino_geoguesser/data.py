...
import torch
from torch.utils.data import Dataset, DataLoader, random_split, TensorDataset, WeightedRandomSampler
...
def create_image_datasets(cfg: Dict, use_image_aug: bool) -> Tuple[Dataset, Dataset, Dataset, Dict[str, int]]:
    ...
    if use_stratification:
        ...
        # Stage 1: Remove classes with only 1 sample from the entire dataset
        ...
        train_indices, val_test_indices, _, y_val_test = train_test_split(
            indices, labels, test_size=val_test_size, random_state=cfg["training"]["seed"], stratify=labels
        )
        ...
        if len(subset_classes_to_move) > 0:
            ...
            cleaned_val_test_indices = val_test_indices
            cleaned_y_val_test = y_val_test
        ...
        if len(cleaned_val_test_indices) > 1:
            ...
            if relative_test_size >= 1.0: val_indices, test_indices = [], cleaned_val_test_indices
            elif relative_test_size <= 0.0: val_indices, test_indices = cleaned_val_test_indices, []
            else: val_indices, test_indices = train_test_split(
                    cleaned_val_test_indices, test_size=relative_test_size, random_state=cfg["training"]["seed"], stratify=cleaned_y_val_test
                )
        else:
        ...
        full_indices = torch.randperm(total_size, generator=generator).tolist()
...
def create_dataloaders(
    ...
    if cfg.get("training", {}).get("use_weighted_sampling", False) and not (precomputed_path and precomputed_path.exists()):
        ...
        # This logic is for the image-based dataset.
        # For EmbeddingDataset, the pre-computation should ideally handle this.
        ...
        weights = [1.0 / class_counts[train_ds.dataset.df.iloc[i][data_cfg.country_column]] for i in train_ds.indices]
