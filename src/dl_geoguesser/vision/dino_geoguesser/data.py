...
def create_image_datasets(cfg: Dict, use_image_aug: bool) -> Tuple[Dataset, Dataset, Dataset, Dict[str, int]]:
    ...
    if use_stratification:
        ...
        if len(cleaned_val_test_indices) > 1:
            ...
            else: val_indices, test_indices = train_test_split(cleaned_val_test_indices, test_size=relative_test_size, random_state=cfg["training"]["seed"], stratify=cleaned_y_val_test)
        else:
    ...
    full_dataset_train = GeoHintsDataset(df, data_cfg.root, country2id, data_cfg.image_column, data_cfg.country_column, data_cfg.content_column, train_transform)
    full_dataset_eval = GeoHintsDataset(df, data_cfg.root, country2id, data_cfg.image_column, data_cfg.country_column, data_cfg.content_column, eval_transform)
